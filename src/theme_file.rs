...
fn config_path() -> Option<PathBuf> {
    let dir = dirs::config_dir()?.join("grove");
    let _ = std::fs::create_dir_all(&dir);
    Some(dir.join("themes.json"))
}
...
impl PastedColors {
    fn set(&mut self, field: &'static str, color: Color) {
        ...
        self.colors.sort_by_key(|(f, _)| FIELD_ORDER.iter().position(|x| x == f).unwrap_or(99));
    }

    pub fn get(&self, field: &str) -> Option<Color> {
        self.colors.iter().find(|(f, _)| *f == field).map(|(_, c)| *c)
    }
}
...
fn parse_paste_json(trimmed: &str) -> Result<PastedColors, String> {
    ...
    let colors_obj = match obj.get("colors") {
        Some(v) => v.as_object().ok_or_else(|| "\"colors\" must be an object".to_string())?,
    ...
    for (key, value) in colors_obj {
        ...
        out.colors.push((canonical, parse_hex(hex).map_err(|e| format!("field '{key}': {e}"))?));
    }
    out.colors.sort_by_key(|(f, _)| FIELD_ORDER.iter().position(|x| x == f).unwrap_or(99));
...
mod tests {
    ...
    fn parse_hex_ok_with_and_without_hash() {
        assert!(matches!(parse_hex("#ff00aa"), Ok(Color::Rgb(0xff, 0x00, 0xaa))));
        assert!(matches!(parse_hex("FF00AA"), Ok(Color::Rgb(0xff, 0x00, 0xaa))));
    }
    ...
    fn parse_hex_incremental_typing_states() {
        ...
        assert!(parse_hex("#a1b2cg").is_err(), "a non-hex digit must stay invalid");
        assert!(parse_hex("#a1b2c30").is_err(), "trailing extra digit must stay invalid");
    }
    ...
    fn apply_pasted_colors_subset_updates_only_named_fields() {
        ...
        assert_eq!(draft.fg, original_fg, "untouched fields must survive a subset apply");
        assert_eq!(draft.name, "mytheme", "name untouched when the paste had none");
    }
