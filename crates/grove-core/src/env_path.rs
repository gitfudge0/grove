...
mod tests {
    ...
    // These two mutate the process-global `PATH` env var, which
    // `cargo test`'s concurrent test threads would otherwise race over.
    // Serialized behind a mutex, same pattern as `theme.rs`'s
    // `CUSTOM_TEST_LOCK`. Each also builds its fixture directory under a
    // fresh `tempfile::TempDir` rather than a fixed name in the system temp
    // dir, so concurrent or interrupted runs never collide on disk.
    #[cfg(windows)]
