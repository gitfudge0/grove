...
/// Merges into the last mark if it already covers the full run, otherwise adds a new one; empty runs are skipped so callers can flush freely.
fn push_merged(
...
/// Nested child marks are shifted to match the combined text so `**_x_**` renders as both bold and italic; inline images split the run and become sibling nodes.
    ...
    for child in children {
        ...
        for node in child_paragraph.children {
            ...
            if let Some(mut image) = node.image {
                ...
                // gpui's InteractiveText doesn't support inline images, so flush the accumulated run and emit the image as its own sibling.
                push_merged(
                    paragraph,
