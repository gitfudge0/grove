...
/// Runs during AST conversion, often on a background task; must not depend on [`Window`]/[`App`].
...
pub trait MarkdownPlugin: Send + Sync + 'static {
    /// Plugins are inline by default; TextView doesn't support inline custom rendering yet, so block plugins must return `true`.
...
impl<'a> MarkdownParseContext<'a> {
    ...
    /// Byte offset of `source` in the full document when parsing an appended fragment.
...
impl MarkdownExtensions {
    /// Disables raw HTML constructs, since markdown-rs gives HTML priority over MDX when both are enabled.
