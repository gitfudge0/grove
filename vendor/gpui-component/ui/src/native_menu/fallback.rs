//! Fallback popup menu for platforms without an OS-native popup (e.g. Linux). It renders gpui-component's drawn [`PopupMenu`] through an overlay held by [`Root`]. Unlike a real native menu it is clipped to the window, but it keeps the [`super::NativeMenu`] API working on every platform.
...
impl FallbackMenuOverlay {
    ...
    /// Build a [`PopupMenu`] from `items` and show it anchored at `position`. `action_context` is the focus handle the selected action is dispatched to, matching the native backends (which dispatch to the window's focus).
