...
/// AppKit doesn't resize the image to fit the row, so a large file would overflow it.
const MENU_IMAGE_SIZE: f64 = 16.0;

/// The tag of the selected item, or `-1` if none.
struct MenuTargetIvars {
    selected: Cell<isize>,
}

define_class!(
    // A throwaway object that receives the menu item action and records which item (by tag) was clicked.
...
/// Run from a foreground task so gpui is not borrowed while the AppKit tracking loop runs.
pub(super) fn show(
    ...
    // gpui's inherent `Window::window_handle`, not the `raw_window_handle::HasWindowHandle` trait method in scope below.
    ...
    cx.spawn(async move |cx| {
        ...
        let _ = cx.update(move |app| {
            let _ = handle.update(app, move |_, window, app| {
                ...
                // Without this, a dismissed menu leaves the window idle and unresponsive to a second right-click.
                window.refresh();
            });
        });
    })
...
fn run_menu(
    ...
    // SAFETY: `view_ptr` came from the window's AppKit handle, and the window outlives this call.
    ...
    // gpui's `position` is top-left origin; AppKit view coords are bottom-left, so flip y.
...
/// Each actionable leaf item's tag equals its index in `actions`, so the selected tag maps back to its action.
fn build_menu<'a>(
        ...
        match item {
            ...
            NativeMenuItem::Item {
                ...
                unsafe {
                    ...
                    if *checked {
                        ns_item.setState(1); // NSControlStateValueOn
                    }
                    if let Some(action) = action {
                        if !*disabled {
